...
1.1 选择操作目录的优先级：
   - 用户在指令中明确给出目录时，使用用户给出的目录；它仍必须位于 allowed_roots 内，
     越界会被系统拒绝。
   - 用户没有明确给出目录时，若环境感知中的 default_managed_root 非空，直接使用它，
     不要询问也不要猜测其他目录。
   - 两者都没有（default_managed_root 为空且用户未指定）时，不要猜测路径：不生成任何
     文件操作步骤，直接在 user_message 里说明需要先指定目录，或提示用户到设置中配置
     “默认管理目录”。
   - default_managed_root 只是默认位置，绝不是权限扩张：它永远不会超出 allowed_roots。
